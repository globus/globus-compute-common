...
def _requires_s3_bucket(compute_s3_bucket):
    if not compute_s3_bucket:
...
def test_s3_storage(compute_s3_bucket):
...
def test_s3_storage_payload(compute_s3_bucket):
    """Confirm that data is stored to s3"""
    # We are setting threshold of 0 to force only s3 storage
    store = RedisS3Storage(bucket_name=compute_s3_bucket, redis_threshold=0)
    payload = "Hello World!"
...
def test_s3_storage_below_threshold(compute_s3_bucket):
    """Confirm that data is NOT stored to s3"""
    store = RedisS3Storage(bucket_name=compute_s3_bucket, redis_threshold=100)
    data = "Hello World!"
    task = SimpleInMemoryTask()
    ...
    response = s3_client.list_objects(Bucket=compute_s3_bucket, Prefix=task.task_id)
...
def test_s3_storage_bad_bucket():
    ...
    bucket_name = f"compute-nonexistent-bucket-{uuid.uuid4().hex}"
...
def test_s3_storage_direct(compute_s3_bucket):
...
def test_s3_storage_direct_payload(compute_s3_bucket):
...
def test_s3_storage_deleted_data(compute_s3_bucket):
    store = RedisS3Storage(bucket_name=compute_s3_bucket, redis_threshold=0)
    result = "Hello World!"
    task = SimpleInMemoryTask()
...
def test_s3_storage_deleted_payload(compute_s3_bucket):
    store = RedisS3Storage(bucket_name=compute_s3_bucket, redis_threshold=0)
    data = "Hello World!"
    task = SimpleInMemoryTask()
